...
function updateBatteryWidgetText(acpi_battery_names, battery_sub_widget)
  ...
  function(stdout, stderr, exitreason, exitcode)
    ...
    local low_battery = total_battery_percent < LOW_BATTERY_THRESHOLD

    if low_battery then
      widget_text = "<span color='#ff638f'>" .. widget_text .. "</span>"
    end

    battery_sub_widget.markup = widget_text
  end
  )
end
...
function updateWidgetText(battery_sub_widget)
  ...
  local low_battery = total_battery_percent < LOW_BATTERY_THRESHOLD
  if not isCharging() and should_show_battery_warning and low_battery then
